...
#### 활성화 함수
- 활성화 함수 - 입력 신호를 출력 신호로 변환시켜주는 함수
...
- 경사하강법 종류
    - `확률적 경사하강법` : 주어진 간격대로 1개씩 꺼내서 하강시키는 방법
        - 'batch_size' 배치크기 1, 한개의 샘플로 손실계산. 빠르고
        - sklearn에서는 확률적 경사하강법만 지원
...
- `교차검증`
    - 검증세트 만들면서 훈련세트 데이터수 줄어듬
    - 검증세트를 떼어 내어 검증(평가)하는 과정을 여러번 반복하는 것

    <img src="./image/ml0008.png" width="600">

    - 교차검증 클래스 사용해서 처리 : sklearn.model_selection.cross_validate, sklearn.model_selection.StratifiedKFold
